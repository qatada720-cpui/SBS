...
const AHMED_OPENING =
  "Hi — I'm Ahmed AI at SafeBusinessSelling. I'll keep asking questions until you're ready — use the button below whenever you want to see your matches. What kind of business are you looking to buy?";
...
/** Ahmed keeps cycling through these — no fixed end. */
const QUESTION_POOL: ChatQuestion[] = [
  {
    bot: "What's your total budget for the acquisition?",
    quickReplies: ['< €500k', '€500k – €1M', '€1M – €5M', '€5M – €10M', '€10M+'],
  },
  {
    bot: 'Where should the business be located? Be as specific or broad as you like.',
    placeholder: 'e.g. Within 2h of Amsterdam, BeNeLux, or EU-wide',
  ...
  {
    bot: 'When would you ideally like to close?',
    quickReplies: ['Within 90 days', '3 – 6 months', '6 – 12 months', 'Whenever the right deal'],
  },
  {
    bot: 'Any must-haves or deal-breakers? Sector preferences, team size, revenue model…',
    placeholder: 'e.g. Recurring revenue, €100k+ EBITDA, owner stays 12 months…',
    quickReplies: ['Recurring revenue only', 'B2B preferred', 'No real-estate-heavy', 'Open to any sector'],
  },
  {
    bot: 'What sector experience are you bringing to the deal?',
    quickReplies: ['Same sector operator', 'Adjacent industry', 'Investor / strategic', 'First-time buyer'],
  },
  {
    bot: 'Preferred business size by annual revenue?',
    quickReplies: ['< €500K', '€500K – €1M', '€1M – €5M', '€5M+', 'Flexible'],
  },
  {
    bot: 'Would you consider a phased ownership structure (seller stays on during handover)?',
    quickReplies: ['Yes — strongly prefer phased', 'Open to it', 'Prefer full transfer at close', 'Not sure yet'],
  },
];
...
export function HomePage() {
  ...
  const current = QUESTION_POOL[questionIndex % QUESTION_POOL.length];
  ...
  useEffect(() => { if (!navigating) textareaRef.current?.focus(); }, [questionIndex, thinking, navigating]);
  ...
  function sendAnswer(text: string) {
    ...
    const nextQ = QUESTION_POOL[nextIndex % QUESTION_POOL.length];

    setTimeout(() => {
      setMessages((m) => [...m, { from: 'bot', text: nextQ.bot }]);
      setQuestionIndex(nextIndex);
      setThinking(false);
    }, 700);
  }

  function showMatchingBusinesses() {
    ...
    setTimeout(() => router.push('/ai-match'), 1200);
  }
    ...
    <div className="page-enter" style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '8px 24px 32px', display: 'flex', justifyContent: 'center', background: 'linear-gradient(to top, var(--bg) 60%, transparent)' }}>
        <div className="col gap-3" style={{ width: '100%', maxWidth: 680 }}>
          ...
          {/* Quick replies */}
          {!navigating && current.quickReplies && (
            <div className="row gap-2" style={{ flexWrap: 'wrap' }}>
              {current.quickReplies.map((q) => (
                <button key={q} type="button" className="chip" onClick={() => sendAnswer(q)}>
                  {q}
                </button>
              ))}
            </div>
