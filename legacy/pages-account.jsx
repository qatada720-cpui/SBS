// Account pages: Seller Onboarding, Seller Dashboard, Buyer Dashboard

// ───────────────────────── Seller Onboarding ─────────────────────────
function SellerOnboardingPage({ onNav }) {
          ...
          <div className="col gap-6" style={{ flex: 1, maxWidth: 720 }}>
                ...
                <div className="row gap-3">
                  <Field label="Sector">
                    <select value={form.sector} onChange={e => setForm({ ...form, sector: e.target.value })}>
                      {window.SBS_DATA.SECTORS.slice(1).map(s => <option key={s}>{s}</option>)}
                    </select>
                  </Field>
            ...
            {cur.id === 'verification' && (
              <div className="col gap-3">
                {[
                  { k: 'financialsUploaded', label: 'Last 3 years of financials', desc: 'P&L, balance sheet, cash flow. PDF or XLSX.', sample: 'financials-2023-2025.pdf · 2.4 MB' },
                  { k: 'taxFilingsUploaded', label: 'Tax filings', desc: 'Last 3 fiscal-year corporate tax filings.', sample: 'tax-filings-bundle.pdf · 1.1 MB' },
                ].map(u => (
            ...
            {cur.id === 'phased' && (
              <div className="col gap-4">
                ...
                <div className="card" style={{ padding: 28 }}>
                  <PhaseTracker phases={window.SBS_DATA.PHASES} currentPhase={1} compact />
                </div>

                <div className="col gap-2">
                  {window.SBS_DATA.PHASES.map((p, i) => (
              ...
              <div className="col gap-6">
                <div className="card col gap-4" style={{ padding: 24 }}>
                  ...
                  <div className="row gap-2 muted" style={{ fontSize: 13 }}>
                    <span>{form.sector}</span><Icon.Dot size={3} /><span>{form.location}</span><Icon.Dot size={3} /><span>€{(form.revenue/1000000).toFixed(1)}M revenue</span>
                  </div>
                ...
                <Button variant="primary" size="lg" onClick={() => onNav('seller-dashboard')} iconRight={<Icon.Arrow size={14} />}>
...
// ───────────────────────── Seller Dashboard ─────────────────────────
function SellerDashboardPage({ onNav }) {
  ...
  return (
    <div className="page-enter" style={{ background: 'var(--bg)' }}>
      <section style={{ padding: '32px 0 0' }}>
        <div className="container row" style={{ justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div className="col gap-2">
            <SectionEyebrow>Seller dashboard</SectionEyebrow>
            <h1 style={{ fontSize: 36 }}>Northwind Logistics</h1>
          </div>
          <div className="row gap-3">
            <Button variant="secondary" size="sm" onClick={() => onNav('listing', 'l-001')}>View public listing</Button>
            <Button variant="primary" size="sm" onClick={() => onNav('seller-onboarding')}>Edit listing</Button>
          </div>
        </div>
      </section>
                  ...
                  {activeDeals.map((d, i) => (
                    <div key={i} className="row hair" style={{ padding: 16, borderRadius: 8, gap: 24, alignItems: 'center' }}>
                      ...
                      <div className="col" style={{ flex: 1, minWidth: 0, gap: 6 }}>
                        <PhaseTracker phases={window.SBS_DATA.PHASES} currentPhase={d.phase} compact />
                      </div>
            ...
            <div className="col gap-4">
              {activeDeals.map((d, i) => (
                <div key={i} className="card col gap-6" style={{ padding: 28 }}>
                  ...
                  <PhaseTracker phases={window.SBS_DATA.PHASES} currentPhase={d.phase} />
...
// ───────────────────────── Buyer Dashboard ─────────────────────────
function BuyerDashboardPage({ onNav }) {
  const [section, setSection] = React.useState('matches');
  const { MATCHES, LISTINGS } = window.SBS_DATA;
  ...
  return (
    <div className="page-enter">
      <section style={{ padding: '32px 0 0' }}>
        <div className="container row" style={{ justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div className="col gap-2">
            <SectionEyebrow>Buyer dashboard</SectionEyebrow>
            <h1 style={{ fontSize: 36 }}>Welcome back, Marek</h1>
          </div>
          <div className="row gap-3">
            <Button variant="secondary" size="sm" onClick={() => onNav('ai-match')}>Run AI match</Button>
            <Button variant="primary" size="sm" onClick={() => onNav('marketplace')} iconRight={<Icon.Arrow size={12} />}>Browse marketplace</Button>
          </div>
        </div>
      </section>
        ...
        <div className="container">
          {section === 'matches' && (
            <div className="col gap-4">
              <div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-end' }}>
                ...
                <Button variant="ghost" size="sm" onClick={() => onNav('ai-match')}>Refine criteria →</Button>
              </div>
                  ...
                  <div key={m.listing.id} className="card row" style={{ padding: 0, overflow: 'hidden' }}>
                    ...
                    <div className="col gap-3" style={{ flex: 1, padding: 24 }}>
                      ...
                      <div className="row gap-2" style={{ marginTop: 4 }}>
                        <Button variant="primary" size="sm" onClick={() => onNav('listing', m.listing.id)}>View listing</Button>
              ...
              {deals.map((d, i) => (
                <div key={i} className="card col gap-6" style={{ padding: 28 }}>
                  <div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <Button variant="secondary" size="sm" iconRight={<Icon.ArrowUpRight size={11} />} onClick={() => onNav('listing', d.listing.id)}>Open deal</Button>
                  </div>
          ...
          {section === 'saved' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
              {LISTINGS.slice(0, 6).map(l => <ListingCard key={l.id} listing={l} onClick={() => onNav('listing', l.id)} />)}
            </div>
          )}
        </div>
      </section>
...
Object.assign(window, { SellerOnboardingPage, SellerDashboardPage, BuyerDashboardPage });
