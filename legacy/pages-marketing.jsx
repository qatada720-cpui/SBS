// Marketing pages: Home, How It Works, About, Contact

// ───────────────────────── Home (Conversational AI) ─────────────────────────
const CONVERSATION = [
  {
    id: 'intent',
    bot: "Hi — I'm Sage, your acquisition assistant. I'll ask a few questions to find businesses that fit. What kind of business are you looking to buy?",
    placeholder: 'e.g. A profitable bakery in Amsterdam, or a logistics SME with recurring revenue…',
    quickReplies: ['A bakery or café', 'An e-commerce brand', 'A SaaS / tech company', 'A logistics business', 'Not sure yet — show me'],
  },
  {
    id: 'budget',
    bot: "Got it. What's your total budget for the acquisition?",
    quickReplies: ['< €500k', '€500k – €1M', '€1M – €5M', '€5M – €10M', '€10M+'],
  },
  {
    id: 'region',
    bot: "Where do you want the business to be located? You can be specific or broad.",
    placeholder: 'e.g. Within 2h of Amsterdam, or BeNeLux, or remote-friendly',
    quickReplies: ['Netherlands', 'BeNeLux', 'DACH', 'EU-wide', 'Remote-friendly OK'],
  },
  {
    id: 'experience',
    bot: "How involved do you want to be day-to-day after the acquisition?",
    quickReplies: ['Full-time CEO from day one', 'Phased into CEO over 6 months', 'Strategic / chair only', 'Pure financial sponsor'],
  },
  {
    id: 'timeline',
    bot: "When would you like to close?",
    quickReplies: ['Within 90 days', '3 – 6 months', '6 – 12 months', 'Whenever the right deal'],
  },
  {
    id: 'extras',
    bot: "Last one — anything else I should know? Specific requirements, deal-breakers, sectors to avoid, team-size preferences, anything goes.",
    placeholder: 'e.g. Must have €100k+ EBITDA, team of 5+, no consumer-facing brands…',
    quickReplies: ["I'm good — find matches", 'Recurring revenue only', 'Owner can stay 12+ months', 'No real-estate-heavy businesses'],
    final: true,
  },
];
...
function HomePage({ onNav }) {
  const [messages, setMessages] = React.useState([
    { from: 'bot', step: 0, text: CONVERSATION[0].bot },
  ]);
  ...
  React.useEffect(() => { textareaRef.current?.focus(); }, [step, thinking]);
  ...
  function sendAnswer(text) {
    ...
    setTimeout(() => {
      if (current.final) {
        setMessages(m => [
          ...m,
          { from: 'bot', text: "Scanning 2,413 verified businesses against your profile…", searching: true },
        ]);
        setThinking(false);
        setDone(true);
        setTimeout(() => onNav('ai-match'), 1400);
        return;
      }
      setMessages(m => [...m, { from: 'bot', step: next, text: CONVERSATION[next].bot }]);
      setStep(next);
      setThinking(false);
    }, 700);
  }
  ...
  const current = CONVERSATION[step];
  const showMatchesNow = step >= 2 && !done;
    ...
    <div className="page-enter" style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="col" style={{ width: '100%', maxWidth: 680, flex: 1, minHeight: 0 }}>
          ...
          <div className="row gap-3" style={{ alignItems: 'center', paddingBottom: 24 }}>
            ...
            <div className="col" style={{ gap: 1 }}>
              <span style={{ fontSize: 13, fontWeight: 500 }}>Sage</span>
      ...
      <div style={{ padding: '8px 24px 32px', display: 'flex', justifyContent: 'center', background: 'linear-gradient(to top, var(--bg) 60%, transparent)' }}>
        <div className="col gap-3" style={{ width: '100%', maxWidth: 680 }}>
          {/* Quick replies */}
          {!done && current && current.quickReplies && (
            <div className="row gap-2" style={{ flexWrap: 'wrap' }}>
              {current.quickReplies.map(q => (
                <button key={q} className="chip" onClick={() => sendAnswer(q)}>{q}</button>
              ))}
              {showMatchesNow && (
                <button className="chip" onClick={() => onNav('ai-match')} style={{ borderColor: 'var(--blue)', color: 'var(--blue)' }}>
                  Skip — show matches now →
                </button>
              )}
            </div>
          )}
            ...
            <div className={`prompt-box ${input.trim() ? 'has-value' : ''}`} style={{ width: '100%', border: '0.5px solid var(--border-strong)', borderRadius: 14, background: 'var(--surface)' }}>
              ...
              <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center', padding: '8px 8px 8px 16px' }}>
                <span className="muted" style={{ fontSize: 11 }}>
                  Question {step + 1} of {CONVERSATION.length} · ↵ to send
                </span>
                <button
        ...
        <div className="container col gap-4">
          ...
          <div className="row gap-2 hair" style={{ padding: 4, borderRadius: 999, width: 'fit-content', marginTop: 24 }}>
            {[{ id: 'buyer', l: 'For buyers' }, { id: 'seller', l: 'For sellers' }].map(t => (
              <button key={t.id} onClick={() => setSide(t.id)}
                style={{
...
function ContactPage({ onNav }) {
  ...
  const [form, setForm] = React.useState({ name: '', email: '', topic: 'general', msg: '' });
...
Object.assign(window, { HomePage, HowItWorksPage, AboutPage, ContactPage });
